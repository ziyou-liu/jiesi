...
echo"<script>self.location.href='DownDatabaseZip.php?f=$zipname&p=$path';</script>";
}
...
global $empire,$bakpath,$fun_r,$phome_db_ver,$phome_db_dbname;
$dbname=$phome_db_dbname;
...
echo $fun_r['StartToBak']."<script>self.location.href='DatabaseBackupDo.php?phome=$phome&t=0&s=0&p=0&mypath=$add[mypath]&waitbaktime=$waitbaktime';</script>";
exit();
}
...
if($t>=$count)
{
echo"<script>alert('".$fun_r['BakSuccess']."\\n\\n".$fun_r['TotalUseTime'].ToChangeUseTime($stime)."');self.location.href='DatabaseBackup.php';</script>";
exit();
}
...
$empire->free($sql);
echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=DatabaseBackupDo.php?phome=BakExe&s=$s&p=$p&t=$t&mypath=$mypath&alltotal=$num&thenof=$noautof&fieldnum=$fieldnum&stime=$stime&waitbaktime=$waitbaktime&collation=$collation\">".$fun_r['BakOneDataSuccess'].Ebak_EchoBakSt($btb[$t],$count,$t,$num,$s);
exit();
}
}
if(empty($p)||$b==1)
...
$t++;
$empire->free($sql);
echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=DatabaseBackupDo.php?phome=BakExe&s=0&p=0&t=$t&mypath=$mypath&stime=$stime&waitbaktime=$waitbaktime\">".$fun_r['OneTableBakSuccOne'].$btb[$t-1].$fun_r['OneTableBakSuccTwo'];
exit();
}
...
$t++;
$empire->free($sql);
echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=DatabaseBackupDo.php?phome=BakExeT&s=0&p=0&t=$t&mypath=$mypath&stime=$stime&waitbaktime=$waitbaktime\">".$fun_r['OneTableBakSuccOne'].$btb[$t-1].$fun_r['OneTableBakSuccTwo'];
exit();
}
$p++;
$sfile=$path."/".$btb[$t]."_".$p.".php";
...
WriteFiletext_n($sfile,$dumpsql);
$empire->free($sql);
echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=DatabaseBackupDo.php?phome=BakExeT&s=$s&p=$p&t=$t&mypath=$mypath&alltotal=$num&thenof=$noautof&fieldnum=$fieldnum&auf=$auf&aufval=$lastaufval&stime=$stime&waitbaktime=$waitbaktime&collation=$collation\">".$fun_r['BakOneDataSuccess'].Ebak_EchoBakSt($btb[$t],$count,$t,$num,$s);
exit();
}
...
Header("Location:DatabaseRestore.php?mydbname=$b_dbname&mypath=$path");
exit();
}
