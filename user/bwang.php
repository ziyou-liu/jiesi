...
$sqlb="select isbwang from {$db_prefix}users where username='".$_SESSION['glo_username']."'";
$rsb=$db->get_one($sqlb);
if($rsb['isbwang']==0){
die('<center>您还没有进入B网体</center>');
}
...
$preupstr="";
gethypreupstr($hyid,$preupstr);
$preupary=explode(",",$preupstr);
if (!in_array($_SESSION["glo_username"],$preupary)){
...
$sqlu="select rank,realname from {$db_prefix}users where username='".$rs11['username']."'";
$rsu=$db->get_one($sqlu);
$hyrank=$rsu["rank"];
;echo '';
if ($hyid!=$_SESSION["glo_username"]){
...
$sql="select * from {$db_prefix}bnet where prename='".$rs11["username"]."' and pos='$j' ";
$ss2=$db->get_one($sql);
$new=0;
if(!empty($ss2)){
$price=0;
